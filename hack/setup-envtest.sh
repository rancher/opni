...
function fetch_envtest_tools {
  ...
  # ** Begin opni-specific modifications **
  # Download kube-controller-manager to the same location
  curl -sL "https://dl.k8s.io/v${k8s_version}/bin/${goos}/${goarch}/kube-controller-manager" -o "${dest_dir}/bin/kube-controller-manager"
  chmod +x "${dest_dir}/bin/kube-controller-manager"
  # ** End modifications **
}
