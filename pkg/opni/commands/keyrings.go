...
func BuildKeyringsGetCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, doc := range resp.Documents {
				if d, err := config.DecodeDocument[*v1beta1.GatewayConfig](doc.Json); err == nil {
					gatewayConf = d
					break
				}
			}
			...
			store, err := backend.KeyringStore(cmd.Context(), "gateway", &core.Reference{
				Id: args[0],
			})
			if err != nil {
			...
			default:
				return fmt.Errorf("unsupported encoding: %s", format)
			}
	...
	cmd.Flags().StringVarP(&format, "format", "f", "json", "Output format. One of: json|base64|crd")
