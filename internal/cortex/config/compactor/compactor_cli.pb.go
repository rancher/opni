...
func (in *Config) FlagSet(prefix ...string) *pflag.FlagSet {
	...
	fs.Var(flagutil.IntPtrValue(&in.BlockSyncConcurrency), strings.Join(append(prefix, "block-sync-concurrency"), "."), "Number of Go routines to use when syncing block index and chunks files from the long term storage.")
	fs.Var(flagutil.IntPtrValue(&in.MetaSyncConcurrency), strings.Join(append(prefix, "meta-sync-concurrency"), "."), "Number of Go routines to use when syncing block meta files from the long term storage.")
	...
	fs.Var(flagutil.IntPtrValue(&in.CompactionRetries), strings.Join(append(prefix, "compaction-retries"), "."), "How many times to retry a failed compaction within a single compaction run.")
	fs.Var(flagutil.IntPtrValue(&in.CompactionConcurrency), strings.Join(append(prefix, "compaction-concurrency"), "."), "Max number of concurrent compactions running.")
	...
	fs.Var(flagutil.IntPtrValue(&in.CleanupConcurrency), strings.Join(append(prefix, "cleanup-concurrency"), "."), "Max number of tenants for which blocks cleanup and maintenance should run concurrently.")
	...
	fs.Var(flagutil.BoolPtrValue(&in.SkipBlocksWithOutOfOrderChunksEnabled), strings.Join(append(prefix, "skip-blocks-with-out-of-order-chunks-enabled"), "."), "When enabled, mark blocks containing index with out-of-order chunks for no compact instead of halting the compaction.")
	fs.Var(flagutil.IntPtrValue(&in.BlockFilesConcurrency), strings.Join(append(prefix, "block-files-concurrency"), "."), "Number of goroutines to use when fetching/uploading block files from object storage.")
	fs.Var(flagutil.IntPtrValue(&in.BlocksFetchConcurrency), strings.Join(append(prefix, "blocks-fetch-concurrency"), "."), "Number of goroutines to use when fetching blocks from object storage when compacting.")
	fs.Var(flagutil.BoolPtrValue(&in.BlockDeletionMarksMigrationEnabled), strings.Join(append(prefix, "block-deletion-marks-migration-enabled"), "."), "When enabled, at compactor startup the bucket will be scanned and all found deletion marks inside the block location will be copied to the markers global location too. This option can (and should) be safely disabled as soon as the compactor has successfully run at least once.")
	...
	fs.Var(flagutil.BoolPtrValue(&in.AcceptMalformedIndex), strings.Join(append(prefix, "accept-malformed-index"), "."), "When enabled, index verification will ignore out of order label names.")
	return fs
}
