...
async def pull_metrics_data(end_time, service, user_id:str, metrics, ns):
    ...
    res = []
    for m in metrics:
        q1 = await metric_queryrange(service, user_id, m,end_time=end_time, time_delta=timedelta(minutes=FULL_WINDOW_SIZE),step_minute=1, namespace=ns)
        res.append(q1)
    return res
...
def filter_metrics(q1 ,m_name, is_debug = False):
    ...
    for r in q1["result"]:
        ...
        history, evaluate_window, test_window = values0[:HISTORY_WINDOW_SIZE], values0[-PATTERN_WINDOW_SIZE:-TEST_WINDOW_SIZE], values0[-TEST_WINDOW_SIZE:]
...
async def get_abnormal_metrics(service, cluster_id, requested_ts: datetime= None, ns:str="default"):
    '''
