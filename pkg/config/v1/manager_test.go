...
var _ = Describe("Gateway Config Manager", Label("unit"), func() {
	...
	defaultStore := inmemory.NewValueStore[*GatewayConfigSpec](util.ProtoClone)
	activeStore := inmemory.NewValueStore[*GatewayConfigSpec](util.ProtoClone)

	BeforeEach(func() {
		mgr = NewGatewayConfigManager(defaultStore, activeStore, flagutil.LoadDefaults)
	...
	It("should create reactive messages", func(ctx SpecContext) {
		msg := &GatewayConfigSpec{}
		rand := protorand.New[*GatewayConfigSpec]()
		...
		verifyWatches := func(spec *GatewayConfigSpec, ws []<-chan protoreflect.Value, pathsToCheck ...map[string]struct{}) {
			recvFailures := []error{}
		ALL_PATHS:
		...
		spec := rand.MustGen()
		_, err := mgr.SetConfiguration(ctx, &SetRequest{
			Spec: spec,
		})
