...
func (c *InternalConditionEvaluator[T]) EvaluateLoop() {
	...
	for {
		...
		case <-c.evaluationCtx.Done():
			return
		case <-ticker.C:
			lastKnownState, err := c.storageNode.StateCache.Get(c.evaluationCtx, c.conditionId)
			if err != nil {
				continue
			}
			if lastKnownState.Healthy {
				...
				if interval > c.evaluateDuration {
					...
					if !c.IsFiring() {
						...
						err = c.storageNode.IncidentCache.OpenInterval(c.evaluationCtx, c.conditionId, timestamppb.Now())
						if err != nil {
							c.lg.Error(err)
						}
					}
				...
				c.SetFiring(false)
				err = c.storageNode.IncidentCache.CloseInterval(c.evaluationCtx, c.conditionId, timestamppb.Now())
				if err != nil {
					c.lg.Error(err)
				}
...
func (c *InternalConditionEvaluator[T]) UpdateState(ctx context.Context, s *alertingv1.CachedState) error {
	...
	if c.storageNode.StateCache.IsDiff(ctx, c.conditionId, s) {
		return c.storageNode.StateCache.Put(ctx, c.conditionId, s)
	}
	return nil
}
...
func (c *InternalConditionEvaluator[T]) CalculateInitialState() {
	...
	if _, getErr := c.storageNode.IncidentCache.Get(c.evaluationCtx, c.conditionId); errors.Is(nats.ErrKeyNotFound, getErr) {
		err := c.storageNode.IncidentCache.Put(c.evaluationCtx, c.conditionId, alertingv1.NewIncidentIntervals())
		if err != nil {
			c.lg.Error(err)
			c.cancelEvaluation()
			return
	...
	if st, getErr := c.storageNode.StateCache.Get(c.evaluationCtx, c.conditionId); errors.Is(nats.ErrKeyNotFound, getErr) {
		if err := c.storageNode.StateCache.Put(c.evaluationCtx, c.conditionId, incomingState); err != nil {
			c.cancelEvaluation()
			return
		}
