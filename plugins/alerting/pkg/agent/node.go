...
func (s *AlertingNode) doSync(ctx context.Context) {
	...
	lg.Debug(fmt.Sprintf("syncing %s node", s.capability))
	...
	case corev1.ConfigStatus_UpToDate:
		lg.Info(fmt.Sprintf("%s node config is up to date", s.capability))
	case corev1.ConfigStatus_NeedsUpdate:
		lg.Info(fmt.Sprintf("%s updating node config", s.capability))
...
func (s *AlertingNode) updateConfig(ctx context.Context, config *node.AlertingCapabilityConfig) error {
	...
	if err != nil {
		lg.With(logger.Err(err)).Error(fmt.Sprintf("failed to fetch %s node id %s", s.capability, err))
		return err
	}
	s.idMu.RUnlock()
	...
	if err := eg.Error(); err != nil {
		...
		lg.With(logger.Err(err)).Error(fmt.Sprintf("%s node configuration error", s.capability))
		return err
	} else {
		s.config = config
	}
...
func (s *AlertingNode) sendHealthUpdate() {
	...
	if !s.hasRemoteHealthClient() {
		lg.Warn(fmt.Sprintf("failed to send %s node health update, remote health client not set", s.capability))
		return
	}

	health, err := s.GetHealth(s.ctx, &emptypb.Empty{})
	if err != nil {
		lg.With(logger.Err(err)).Warn(fmt.Sprintf("failed to get %s node health", s.capability))
		return
	}

	if _, err := s.healthListenerClient.UpdateHealth(s.ctx, health); err != nil {
		lg.With(logger.Err(err)).Warn(fmt.Sprintf("failed to send %s node health updates", s.capability))
	} else {
		lg.Debug("send node health update")
	}
}
