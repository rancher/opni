...
// Copied from github.com/distribution/distribution/v3/reference/regexp.go
// File also includes some updated logic for domain names
...
var (
	...
	domainName = `(?:` + localhost + `|` + fqdn + `)`

	fqdn = domainNameComponent + oneOrMore(`\.`+domainNameComponent)
...
// oneOrMore wrapes the expression in a noncapturing group that must occur
