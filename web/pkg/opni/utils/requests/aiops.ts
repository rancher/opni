...
export async function getAISettings(): Promise<AISettings | null> {
  try {
    return (await axios.get<AISettings>('opni-api/aiops/AIAdmin/ai/settings')).data;
  } catch (ex) {
    return null;
  }
}
...
export async function deleteAISettings() {
  await axios.delete('opni-api/aiops/AIAdmin/ai/settings');
}

export async function updateAISettings(settings: AISettings) {
  await axios.put<AISettings>('opni-api/aiops/AIAdmin/ai/settings', settings);
}

export async function isUpgradeAvailable(): Promise<boolean> {
  return (await axios.get<UpgradeAvailableResponse>('opni-api/aiops/AIAdmin/ai/upgrade')).data.UpgradePending;
}

export async function upgrade() {
  await axios.post('opni-api/aiops/AIAdmin/ai/upgrade');
}

export async function getRuntimeClasses(): Promise<RuntimeClassResponse> {
  const r = (await axios.get<RuntimeClassResponse>('opni-api/aiops/AIAdmin/ai/runtimeclasses')).data;
