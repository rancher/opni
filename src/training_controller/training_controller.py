...
nulog_spec = {"name": "nulog-train", "container_name": "nulog-train", "image_name": "amartyarancher/nulog-train","image_pull_policy": "Always", "labels": {"app": "nulog-train"}, "restart_policy": "Never","requests": {"memory": "1Gi", "cpu": 1}, "limits": {"nvidia.com/gpu": 1}, "env": []}
nulog_spec["env"] = [client.V1EnvVar(name="MINIO_ACCESS_KEY", value=MINIO_ACCESS_KEY),client.V1EnvVar(name="MINIO_SECRET_KEY", value=MINIO_SECRET_KEY),client.V1EnvVar(name="NATS_SERVER_URL", value=NATS_SERVER_URL)]
startup_time = time.time()

def job_not_currently_running(job_name, namespace="default"):
    try:
        jobs = api_instance.list_namespaced_job(namespace, pretty=True, timeout_seconds=60)
        ...
        try:
            if pod.status.phase == phase:
                ...
                if "nulog-train" in podname:
                    signals_queue_payload = {"model": "nulog-train", "signal": "finish", "payload": None}
...
def run_job(job_details):
    resource_specifications = client.V1ResourceRequirements(requests=job_details["requests"], limits=job_details["limits"])
    container = client.V1Container(name=job_details["container_name"], image=job_details["image_name"], image_pull_policy=job_details["image_pull_policy"],env=job_details["env"], resources = resource_specifications)
    template = client.V1PodTemplateSpec(metadata=client.V1ObjectMeta(labels=job_details["labels"]), spec=client.V1PodSpec(restart_policy=job_details["restart_policy"], containers=[container]))
    spec = client.V1JobSpec(template=template, backoff_limit=4)
    job = client.V1Job(api_version="batch/v1",kind="Job",metadata=client.V1ObjectMeta(name=job_details["name"]),spec=spec)
        ...
        if signal == "start":
            if job_not_currently_running(model_to_train):
                ...
                if model_to_train == "nulog-train":
                    ...
                    logging.info("Nulog model currently being trained. Job will run after this model's training has been completed")
        else:
    ...
    while True:
        ...
        try:
            ...
            if decoded_payload["model_to_train"] == "nulog":
                signals_queue_payload = {"model": "nulog-train", "signal": "start", "payload": decoded_payload}
                await signals_queue.put(signals_queue_payload)
...
if __name__ == "__main__":
    ...
    consume_nats_drain_signal_coroutine = consume_nats_drain_signal(queue, signals_queue)
    ...
    loop.run_until_complete(asyncio.gather(consumer_coroutine, consume_nats_drain_signal_coroutine, clear_jobs_coroutine, manage_kubernetes_jobs_coroutine))
    try:
        loop.run_forever()
    finally:
        loop.close()
