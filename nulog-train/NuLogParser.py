...
# class logparser
class LogParser:
    def __init__(self, outdir, filters, k, log_format, logName="nulog_model_latest"):
    ...
    def train(
        ...
        logging.info("learning rate : " + str(self.lr))
        ## test!
    ...
    def predict(self, data_tokenized, output_prefix=""):
        ...
        for i, (x, y, ind) in enumerate(results):
            ...
            for j in range(len(x)):
                if not self.num_there(
                    self.tokenizer.index2word[y[j]]
                ):  ## only consider tokens without digit
        ...
        with open(self.savePath + output_prefix + "anomaly_scores.txt", "w") as fout:
            for j in anomaly_preds:
                fout.write(str(j))
                fout.write("\n")
    ...
    def run_test(self, dataloader, model, loss_compute):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                if i % self.step_size == 1:
                    logging.info("Epoch Step: %d / %d" % (i, len(dataloader)))
