...
export default {
  ...
  computed: {
    ...
    installCommand() {
      ...
      return `helm -n ${ this.namespace } install opni-agent ${ imageMain } ${ prometheus } --set address=${ this.gatewayAddress } --set pin=${ this.pin } --set token=${ this.token } --set friendlyName=${ this.friendlyName } --create-namespace ${ defaultImageRepository }`;
    },
    gatewayUrl() {
