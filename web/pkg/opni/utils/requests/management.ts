...
export async function installCapabilityV2(capability: string, clusterId: string) {
  return (await axios.post(`'opni-api/mgmt/Management/clusters/${ clusterId }/capabilities/${ capability }/install`)).data;
}

export async function uninstallCapabilityStatusV2(capability: string, clusterId: string) {
  return await axios.get(`'opni-api/mgmt/Management/clusters/${ clusterId }/capabilities/${ capability }/uninstall/status`);
}
...
export async function getTokens(vue: any) {
  const tokensResponse = (await axios.get<TokensResponse>(`'opni-api/mgmt/Management/tokens`)).data.items;

  return tokensResponse.map(tokenResponse => new Token(tokenResponse, vue));
}

export async function getDefaultImageRepository() {
  return (await axios.get<DashboardSettings>(`'opni-api/mgmt/Management/dashboard/settings`)).data.global?.defaultImageRepository;
}

export async function getCapabilities(vue: any) {
  const capabilitiesResponse = (await axios.get<any>(`'opni-api/mgmt/Management/capabilities`)).data.items;
...
export function uninstallCapability(clusterId: string, capability: string, deleteStoredData: boolean, vue: any) {
  ...
  return axios.post<any>(`'opni-api/mgmt/Management/clusters/${ clusterId }/capabilities/${ capability }/uninstall`, { options: { ...initialDelay, deleteStoredData } });
}

export async function cancelCapabilityUninstall(clusterId: string, capabilityName: string) {
  await axios.post(`'opni-api/mgmt/Management/clusters/${ clusterId }/capabilities/${ capabilityName }/uninstall/cancel`, {
    name:    capabilityName,
...
export async function uninstallCapabilityStatus(clusterId: string, capability: string, vue: any) {
  return (await axios.get<CapabilityStatusResponse>(`'opni-api/mgmt/Management/clusters/${ clusterId }/capabilities/${ capability }/uninstall/status`)).data;
}

export async function getCapabilityInstaller(capability: string, token: string, pin: string) {
  return (await axios.post<CapabilityInstallerResponse>(`'opni-api/mgmt/Management/capabilities/${ capability }/installer`, {
    token,
    pin,
  })).data.command;
}
...
export async function getCapabilityStatus(clusterId: string, capability: string, vue: any): Promise<NodeCapabilityStatus> {
  return (await axios.get<NodeCapabilityStatus>(`'opni-api/mgmt/Management/clusters/${ clusterId }/capabilities/${ capability }/status`)).data;
}
...
export async function createToken(ttlInSeconds: string, name: string | null, capabilities: any[]) {
  ...
  const raw = (await axios.post<any>(`'opni-api/mgmt/Management/tokens`, {
    ttl: ttlInSeconds,
    ...labels,
    capabilities,
  })).data;
...
export function deleteToken(id: string): Promise<undefined> {
  return axios.delete(`'opni-api/mgmt/Management/tokens/${ id }`);
}
...
export async function getCerts(): Promise<CertResponse[]> {
  return (await axios.get<CertsResponse>(`'opni-api/mgmt/Management/certs`)).data.chain;
}
...
export async function updateCluster(id: string, name: string, labels: { [key: string]: string }) {
  ...
  (await axios.put<any>(`'opni-api/mgmt/Management/clusters/${ id }`, {
    cluster: { id },
    labels
  }));
}
...
export function deleteCluster(id: string): Promise<undefined> {
  return axios.delete(`'opni-api/mgmt/Management/clusters/${ id }`);
}

export async function getRoles(vue: any): Promise<Role[]> {
  const rolesList = (await axios.get<RolesResponse>(`'opni-api/mgmt/Management/rbac/backend/metrics/roles`)).data.items;
...
export async function getRole(id: string, vue: any): Promise<Role> {
  const roleResponse = (await axios.get<RoleResponse>(`'opni-api/mgmt/Management/rbac/backend/metrics/roles/${ id }`)).data;

  return new Role(roleResponse, vue)
}

export function deleteRole(id: string): Promise<undefined> {
  return axios.delete(`'opni-api/mgmt/Management/rbac/backend/metrics/roles/${ id }`);
}
...
export async function createRole(name: string, clusterIDs: string[], matchLabels: MatchLabel) {
  ...
  (await axios.post<any>(`'opni-api/mgmt/Management/rbac/backend/metrics/roles`, {
    capability: { name: capability }, role
  }));
}

export async function getRoleBindings(vue: any): Promise<RoleBinding[]> {
  const roleBindingsResponse = (await axios.get<RoleBindingsResponse>(`'opni-api/mgmt/Management/rolebindings`)).data.items;
...
export function deleteRoleBinding(id: string): Promise<undefined> {
  return axios.delete(`'opni-api/mgmt/Management/rolebindings/${ id }`);
}
...
export async function createRoleBinding(name: string, roleName: string, subjects: string[]) {
  ...
  (await axios.post<any>(`'opni-api/mgmt/Management/rolebindings`, {
    id: name, roleId: roleName, subjects, metadata
  }));
}

export async function getGatewayConfig(vue: any): Promise<ConfigDocument[]> {
  const config = (await axios.get<GatewayConfig>(`'opni-api/mgmt/Management/config`)).data;
...
export function updateGatewayConfig(jsonDocuments: string[]): Promise<undefined> {
  ...
  return axios.put(`'opni-api/mgmt/Management/config`, { documents });
}
